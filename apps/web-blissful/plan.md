...
3. Copy existing state: `cp -a apps/shared/blissful-storage/data/* /Volumes/2TB/NAS/blissful/state/blissful-storage/`.
4. Update Compose volumes to point to NAS paths.
5. Restart and verify: `docker compose -f docker-compose.blissful-mac.yml up -d`.

### Rollback

- Keep original `apps/shared/blissful-storage/data/` intact until NAS migration is validated (at least 7 days).
