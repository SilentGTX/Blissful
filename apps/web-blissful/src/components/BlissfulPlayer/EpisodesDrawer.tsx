    ...
    <AnimatePresence>
      {open && type === 'series' && videos && videos.length > 0 ? (
        <motion.div
          ...
          className="absolute inset-0 z-40 flex items-center justify-end gap-3 bg-black/90 p-4 pb-24 md:bg-black/30 md:px-8 md:py-12 [@media(max-height:520px)]:!p-2"
          onClick={onClose}
        >
          <motion.div
            ...
            // On short-height (landscape phone) screens, scale the whole drawer
            // down so the big coverflow cards + controls all fit.
            className="pointer-events-auto flex h-full max-h-[800px] w-[80%] flex-col gap-3 md:w-[520px] [@media(max-height:520px)]:gap-1.5 [@media(max-height:520px)]:[zoom:0.8] [@media(max-height:400px)]:[zoom:0.7]"
