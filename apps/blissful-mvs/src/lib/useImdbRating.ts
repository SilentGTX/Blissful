...
/** Ratings are resolved AND cached server-side by the backend
 *  (`/imdb-rating`: Cinemeta `imdbRating` → TMDB `vote_average`, keyed by
 *  IMDB id, ~24h NAS-cached). The client just asks once and layers its own
 *  in-memory + sessionStorage cache on top, so a grid of cards costs at most
 *  one tiny request per title — and zero on revisit. This replaced the old
 *  per-client chain (scrape www.imdb.com → Cinemeta → TMDB-with-user-key);
 *  moving it server-side also means the TMDB fallback works for everyone
 *  (proxy key) and is cheap enough to run on TV (so posters always get a
 *  rating, not just ones whose addon meta already carried one). proxyUrl()
 *  keeps it relative on desktop/browser and absolute via the loopback proxy
 *  on Android Tauri. */
async function fetchImdbRating(imdbId: string): Promise<number | null> {
  ...
  try {
    const response = await fetch(proxyUrl(`/imdb-rating?imdbId=${encodeURIComponent(imdbId)}`));
    if (response.ok) {
