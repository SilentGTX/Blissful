...
export function BlissTooltip({
  ...
  // Ref callback (runs at commit, not render): fan the trigger node out
  // to our own measurement ref AND the caller's `triggerRef`. Writing
  // `.current` here is the standard multi-ref-forwarding pattern; the
  // compiler's immutability rule flags it as a render-time mutation,
  // which it isn't (ref callbacks run during commit).
