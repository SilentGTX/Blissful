...
- **Phase 0b** — actual `apps/web-blissful/` React build, with a temporary `/player-spike` route, over libmpv. Lands later in Phase 0.
...
- [ ] `apps/desktop-blissful/resources/mpv-x64/libmpv-2.dll` exists and is LGPL-built
...
```
apps/desktop-blissful/resources/mpv-x64/libmpv-2.dll
```
...
If 0a passes, move on to staging Phase 0b: build `apps/web-blissful/`, add the temporary `/player-spike` route, point the shell's WebView at the local Vite/UI server, and re-run the same ten-box checklist with the real React DOM.
