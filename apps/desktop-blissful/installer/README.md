...
From `apps/desktop-blissful/installer/`:
...
1. `npm ci` + `npm run build` in `apps/web-blissful/` → React `dist/`
2. `cargo build --release --features spike0a` in `apps/desktop-blissful/`
